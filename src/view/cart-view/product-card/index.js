...
import { Box, Button, ButtonIcon, FormControlErrorText, HStack, Image, Input, InputField, Text, VStack } from "@gluestack-ui/themed";
...
export default function ProductCard({ data, index, onUpdateCart, validateData, isValidateDataCart }) {
  ...
  const onChangeInputQuantity = (data) => {
    setProductUpdate({
      ...productUpdate,
      quantity: parseInt(handleCheckEmpty(data)),
    });
    setIsDefaultData(false);
    ...
    <>
      <HStack style={styles.contentCart} justifyContent="space-between">
        <View style={styles.boxImg}>
          <Image style={styles.imgProduct} alt="Ảnh thuốc" source={{ uri: data.avatar }} />
        </View>
        <VStack style={styles.inforProduct} justifyContent="center">
          ...
          <VStack>
            <HStack style={{ marginBottom: Dimensions.get("window").width * 0.00505 }} justifyContent="space-between">
              <VStack>
              ...
              <Button style={styles.btnUpdateQuantity} size="xs" isDisabled={data.isChange} onPress={onPressUpButton}>
                <ButtonIcon>
            ...
            <HStack justifyContent="space-between" alignItems="center">
              ...
              >
                <ButtonIcon>
                  <AntDesign name="caretdown" size={13} color={color.blueSky} />
                </ButtonIcon>
              </Button>
            </HStack>
          </VStack>
