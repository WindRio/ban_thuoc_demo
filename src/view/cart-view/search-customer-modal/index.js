  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <Modal isOpen={isShowModal} onClose={onCloseModal}>
        ...
        <ModalContent bgColor={color.white} style={styles.modalUser}>
          <ModalHeader>
            <Input w={"100%"} variant="underlined">
              ...
              <InputField ref={refInput} placeholder="Tìm kiếm..." onChangeText={onSearch} />
              <InputSlot display={textSearch ? "flex" : "none"} onPress={clearTextSearch} style={{ marginRight: 12 }}>
          ...
          <ModalBody>
            {isEmptyList ? (
              <EmptyDataCommon title={"Thêm khách hàng mới"} isCheckShowButton={true} onHandlePress={() => onOpenModalCreate()} />
            ) : (
              <FlatList
                scrollEnabled={false}
                data={listData}
        ...
        {!isNotification ? null : <ToastNotificationCommon Info="Thêm thành công!!!!!" Description="Đã thêm mới 1 khách hàng" />}
        <CreateCustomerModal
